...
final class FlashbackController {
    ...
    private static func deliver(report: FlashbackReport, webhookURL: URL?) async throws {
        guard let webhookURL else {
            ...
            device: \(report.device.model, privacy: .public) (\(report.device.modelIdentifier, privacy: .public)) / \(report.device.systemName, privacy: .public) \(report.device.systemVersion, privacy: .public)
